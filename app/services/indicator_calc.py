...
class IndicatorEngineV2:
    ...
    def export_json(self, stock_code: str) -> Dict:
        ...
        for dim, indicators_defs in DIMENSION_INDICATORS.items():
            for ind_name in indicators_defs:
                ind_val = row.get(ind_name)
                is_missing = ind_val is None or (isinstance(ind_val, float) and (np.isnan(ind_val) or np.isinf(ind_val)))
                ind_score = score_indicator(ind_name, None if is_missing else ind_val)
                indicator_scores_detail.append(
                    {
                        "dimension": dim,
                        "indicator": ind_name,
                        "value": None if is_missing else ind_val,
                        "score": float(ind_score),
                        "is_missing": bool(is_missing),
                    }
                )

        drags = [d for d in indicator_scores_detail if d["score"] < 60 and not d["is_missing"]]
        drags.sort(key=lambda x: x["score"])
        attribution = drags[:5]
        if len(attribution) < 5:
            missing = [d for d in indicator_scores_detail if d["is_missing"]]
            attribution.extend(missing[: 5 - len(attribution)])
        ...
        result = {
            ...
            "all_indicator_scores": {d["indicator"]: round(float(d["score"]), 2) for d in indicator_scores_detail},
            "data_quality": {
